...
@section('content')
    <div class="py-40 space-y-2 xl:space-y-0 w-4/5 sm:w-2/5 mx-auto">
        <h2 class="text-red-700 pb-4 text-3xl sm:text-5xl font-bold">
            {{ $article->title }}
        </h2>

        <span class="py-8 text-black text-sm">
            {{ $article->user->name }} · {{ $article->created_at->format("M jS Y") }} ·
            <a href="" class="text-black border-b-2 pb-1 border-red-500 hover:text-red-500 transition-all">
                {{ $article->category->name }}
            </a>
        </span>

        <div>
            <p class="font-bold text-black text-md pt-10">
                {{ $article->excerpt }}
            </p>

            <p class="font-normal text-black text-md pt-4 whitespace-pre-line text-left">
                {{ $article->description }}
            </p>
        ...
        <div class="pt-10">
            <div class="border-t-2 border-red-900">
                <h3 class="text-black font-bold pt-10">
                    {{ $article->category->name }}
                </h3>
                <p class="text-black font-normal pt-2 text-sm">
